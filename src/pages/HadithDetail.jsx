...
import { 
  ...
  Clock,
  RotateCcw,
  // ⬇️ Remplacement : éviter 'Scale' qui pose problème
  Scale3d
} from 'lucide-react';
...
const HADITHS_8_15 = [
  { 
    number: 8, 
    arabic_text: 'النص العربي الكامل للحديث الثامن عن الوضوء كما رواه البخاري ومسلم...', 
    french_text: 'Le Prophète ﷺ a décrit le wuḍūʾ en détail, mentionnant chaque étape avec précision...', 
    source: 'Bukhari & Muslim',
    audio_url: null,
    opinions: {
      Hanafi: { ar: 'النص الفقهي الحنفي...', fr: 'Selon l\'école hanafite...' },
      Maliki: { ar: 'النص الفقهي المالكي...', fr: 'Selon l\'école malikite...' },
      Shafi: { ar: 'النص الفقهي الشافعي...', fr: 'Selon l\'école shafi\'ite...' },
      Hanbali: { ar: 'النص الفقهي الحنبلي...', fr: 'Selon l\'école hanbalite...' }
    }
  }
];

const useAuth = () => ({ user: null });
const nextReview = (base, quality) => ({
  ease: base.ease + (quality >= 4 ? 0.1 : -0.2),
  interval_days: quality >= 4 ? base.interval_days + 1 : 0,
    ...
    <div className="flex items-center gap-2">
      {url ? (
        <>
          ...
          <Badge variant="outline" className="hidden sm:inline">0.9x</Badge>
        </>
      ) : (
...
const SCHOOL_COLORS = {
  Hanafi: { gradient: 'from-blue-500 to-blue-600', bg: 'bg-blue-50 dark:bg-blue-950', border: 'border-blue-200 dark:border-blue-800' },
  Maliki: { gradient: 'from-green-500 to-green-600', bg: 'bg-green-50 dark:bg-green-950', border: 'border-green-200 dark:border-green-800' },
  Shafi: { gradient: 'from-purple-500 to-purple-600', bg: 'bg-purple-50 dark:bg-purple-950', border: 'border-purple-200 dark:border-purple-800' },
  Hanbali: { gradient: 'from-amber-500 to-amber-600', bg: 'bg-amber-50 dark:bg-amber-950', border: 'border-amber-200 dark:border-amber-800' }
};

export function HadithDetail() {
  const hadithNumber = 8; // Mock - replace with useHadithNumberFromRouter()
  const { user } = useAuth();
        ...
        <div className="max-w-5xl mx-auto space-y-6">
          ...
          <Card className="border-slate-200 dark:border-slate-700 shadow-xl bg-white dark:bg-slate-800 overflow-hidden">
            ...
            <CardHeader className="relative z-10">
              ...
              <CardDescription>
                Lis attentivement, écoute la récitation, puis mémorise
              </CardDescription>
            </CardHeader>
          ...
          {hadith.opinions && (
            <Card className="border-slate-200 dark:border-slate-700 shadow-xl bg-white dark:bg-slate-800">
              <CardHeader>
                ...
                <CardDescription>
                  Compare les positions des madhāhib sur ce hadith
                </CardDescription>
              </CardHeader>
                ...
                <Tabs defaultValue="Hanafi" className="w-full">
                  <TabsList className="grid grid-cols-4 w-full">
                    {['Hanafi', 'Maliki', 'Shafi', 'Hanbali'].map(school => (
                      ...
                      >
                        {school === 'Hanafi' ? 'Hanafite' : 
                         school === 'Maliki' ? 'Malikite' :
                         school === 'Shafi' ? "Chafi'ite" : 'Hanbalite'}
                      </TabsTrigger>
                    ))}
                  </TabsList>
          ...
          <Card className="border-slate-200 dark:border-slate-700 shadow-xl bg-white dark:bg-slate-800">
            <CardHeader>
              ...
              <CardDescription>
                Évalue ta maîtrise pour optimiser la répétition espacée
              </CardDescription>
            </CardHeader>
